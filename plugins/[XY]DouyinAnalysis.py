...
# ----------------- 修改开始 -----------------
# 原来是 _client = _h.AsyncClient(timeout=10.0)
_client = None

async def get_client():
    """懒加载 httpx.AsyncClient，防止 loop 已关闭导致报错"""
    global _client
...
本群{'已加入' if str(event.group_id) in _whitelist else '未加入'}白名单"""
    ...
    try:
        client = await get_client()   # <<< 使用懒加载 client
...
print("[Xiaoyi_QQ]抖音解析插件已加载（支持白名单提示功能，修复 Event loop is closed 问题）")